/*
 * Based on code by Andre Reinhald and then heavily modified by Zed A. Shaw.
 */
...
RMElement* RadixMap_find(RadixMap* map, uint32_t key_to_find)
{
    int low = 0;
    int high = map->end - 1;
...
int RadixMap_delete(RadixMap* map, RMElement* element)
{
    ...
    if(map->end > 0) { // Don't bother sorting a one element list.
        RadixMap_sort(map);
    }
    map->end--;
    return 0;
